 ...
 * stream. When the new stream is created, the input streams are drained
 * so that all previous events are combined into the reduced output.
 *
...
module.exports = function (initial, streamReductions) {
  var newStream = Stream.of(initial)
  streamReductions.forEach(function (pair) {
    var stream = pair[0]
    var reduceFn = pair[1]

    function reduceStep (v) {
      newStream.put(reduceFn(newStream.extract(), v))
    }

    stream.drain().forEach(reduceStep)
    stream.subscribe(reduceStep)
  })
  return newStream
}
